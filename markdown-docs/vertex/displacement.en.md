...
### Map Base Layers
The available base map layers can assist you in selecting your area of interest (AOI). Displacement products are available in areas where Velocity data is present.
In the top left corner of the map, you may click the checkbox to toggle your desired map layers on or off.

#### Cumulative Velocity
This is a derived layer based on the cumulative deformation captured by an aggregate of short-wavelength displacement layers from the displacement products.
...
Once you have selected an AOI, the results panel will appear. The left panel lists your selected AOIs, labeled as **Series 1, Series 2**, and so on. A spinning loading icon will be displayed next to each series while it is loading.
...
- The **Info** button opens a modal with more information on the OPERA Sentinel-1 Displacement Products.
